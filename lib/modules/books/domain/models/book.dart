...
class Book extends Equatable {
  const Book(
      {this.id, required this.title, this.indexes = const <BookIndex>[]});
  final String? id;
  final String title;
    ...
    return Book(
      ...
      indexes: items is List
          ? items
              .whereType<Map>()
              .map(
                  (item) => BookIndex.fromJson(Map<String, dynamic>.from(item)))
              .toList()
          : const <BookIndex>[],
    );
  }
...
class BookIndex extends Equatable {
  const BookIndex(
      {required this.title, this.page, this.subindexes = const <BookIndex>[]});
  final String title;
  final int? page;
  ...
  BookIndex copyWith({String? title, int? page, List<BookIndex>? subindexes}) =>
      BookIndex(
          title: title ?? this.title,
          page: page ?? this.page,
